...
def pairwise_distance_rfl(features, features_old, query=None, gallery=None, metric=False):
    ...
    y = torch.cat([features_old[f].unsqueeze(0) for f, _, _,_ in gallery], 0)
...
class Evaluator(object):
    ...
    def evaluate_rfl(self, data_loader, query, gallery, metric=None, cmc_flag=False,
                 rerank=False, pre_features=None, cuhk03=False,training_phase=None, old_feat=None):
        if (pre_features is None):
            features, _ = extract_features(self.model, data_loader,training_phase=training_phase)
        else:
            features = pre_features
        ...
        if old_feat is None or not os.path.exists(old_feat):
            ...
            return self.evaluate(data_loader, query, gallery, metric=metric, cmc_flag=cmc_flag,
                               rerank=rerank, pre_features=features, cuhk03=cuhk03, training_phase=training_phase)
        ...
        print('Applying person re-ranking ...')
        distmat_qq = pairwise_distance_rfl(features, features_old, query, query, metric=metric)
        distmat_gg = pairwise_distance_rfl(features, features_old, gallery, gallery, metric=metric)
        distmat = re_ranking(distmat.numpy(), distmat_qq.numpy(), distmat_gg.numpy())
        return evaluate_all(query_features, gallery_features, distmat, query=query, gallery=gallery, cmc_flag=cmc_flag)