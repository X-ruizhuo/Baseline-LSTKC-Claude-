                ...
                with torch.no_grad(), autocast():
                    for start_pos in range(0, feature_tensor.shape[0], batch_size_transform):
                        ...
                        for i in range(start_pos, end_pos):
                            updated_features_dict[feature_keys[i]] = batch_features_trans[i - start_pos].cpu()
...
def decouple_knowledge(model_old, init_loader, epochs=5, lr=0.001):
    ...
    for epoch in range(epochs):
        ...
        for imgs, fnames, pids, cids, domains in init_loader:
            ...
            # 正则化: 保持与原模型接近
            loss_short_reg = 0
            for p1, p2 in zip(model_short.parameters(), model_old.parameters()):
                if p1.requires_grad:
                    loss_short_reg += F.mse_loss(p1, p2)
            ...
            loss_long_total += loss_long.item()
            loss_short_total += loss_short.item()
            num_batches += 1

        print(f"解耦Epoch {epoch+1}/{epochs}: Long Loss={loss_long_total/num_batches:.4f}, "
              f"Short Loss={loss_short_total/num_batches:.4f}")
...
def train_dataset(cfg, args, all_train_sets, all_test_only_sets, set_index, model, model_trans, model_trans2, out_channel, writer,logger_res=None):
    ...
    for epoch in range(0, Epochs):
        ...
        trainer.train(epoch, train_loader,  optimizer, training_phase=set_index + 1,
                      train_iters=len(train_loader), add_num=add_num, old_model=None,
                      scaler=scaler)
        lr_scheduler.step()       
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-dir', type=str, metavar='PATH',
                        default='/home/xiong_project/L-ReID/data/PRID/')
